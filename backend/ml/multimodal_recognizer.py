...
NEW_PEOPLE_FOLDER = os.path.join(BASE_DIR, "..", "uploads", "faces")
...
def add_new_people(folder, app, face_db):
    """
    Scan uploads/faces folder and extract embeddings for all people.
    Updates face_db with new embeddings from uploaded images.
    """
    if not os.path.exists(folder):
        print(f"[Face] ⚠️  Uploads folder not found: {folder}")
        return face_db
    ...
    for person_name in os.listdir(folder):
        ...
        for img_name in os.listdir(pf):
            ...
            try:
                ...
                if faces:
                    ...
                    img_count += 1
                    print(f"[Face]   ✓ {img_name} → embedding extracted")
                else:
                    print(f"[Face]   ⚠️  {img_name} → no face detected")
        ...
        if embs:
            ...
            print(f"[Face] ✅ {person_name}: {img_count} images → embedding averaged & stored")
            updated_count += 1
        elif os.path.exists(pf) and len(os.listdir(pf)) > 0:
            print(f"[Face] ⚠️  {person_name}: folder has files but no valid faces extracted")

    if updated_count > 0:
        print(f"[Face] 📊 Total people updated: {updated_count}")
    else:
        print(f"[Face] ℹ️  No new people added (folder may be empty or already processed)")
...
def main():
    ...
    print("\n" + "="*60)
    print("🧠 INITIALIZING MULTIMODAL RECOGNIZER")
    print("="*60)
    
    print("\n[Init] Loading face recognition model (buffalo_s)...")
    app = FaceAnalysis(name="buffalo_s")
    app.prepare(ctx_id=0, det_size=(640, 640))
    print("[Init] ✅ Face model ready")

    print(f"\n[Init] Loading face embeddings database...")
    print(f"[Init] → Database path: {FACE_DB_PATH}")
    print(f"[Init] → Uploads folder: {NEW_PEOPLE_FOLDER}")
    
    face_db = load_face_db(FACE_DB_PATH)
    print(f"[Init] ✅ Loaded {len(face_db)} existing people from database")
    
    print(f"\n[Init] Scanning uploads/faces folder for new people...")
    face_db = add_new_people(NEW_PEOPLE_FOLDER, app, face_db)
    save_face_db(face_db, FACE_DB_PATH)
    print(f"[Init] ✅ Face database updated: {len(face_db)} total people registered")

    print("\n[Init] Starting video capture...")
    ...
    print("="*60)
    print(f"   Registered People: {list(face_db.keys())}")
...
# ==========================================
# 🔧 UTILITY: Refresh embeddings from uploads
# ==========================================
def refresh_face_embeddings_from_uploads(uploads_folder: str = None, face_db_path: str = None):
    """
    Standalone function to refresh face embeddings from uploads folder.
    Can be called from Flask service or CLI.
    
    Args:
        uploads_folder: Path to uploads/faces folder (default: NEW_PEOPLE_FOLDER)
        face_db_path: Path to face_embeddings.json (default: FACE_DB_PATH)
    
    Returns:
        dict: Updated face_db with {person_name: embedding_list}
    """
    uploads_folder = uploads_folder or NEW_PEOPLE_FOLDER
    face_db_path = face_db_path or FACE_DB_PATH
    
    print("\n[Refresh] 🔄 Refreshing face embeddings from uploads...")
    print(f"[Refresh] → Uploads folder: {uploads_folder}")
    print(f"[Refresh] → Database file: {face_db_path}")
    
    try:
        # Initialize face model if needed
        face_app = FaceAnalysis(name="buffalo_s")
        face_app.prepare(ctx_id=0, det_size=(640, 640))
        print("[Refresh] ✅ Face model loaded")
        
        # Load existing embeddings
        face_db = load_face_db(face_db_path)
        initial_count = len(face_db)
        print(f"[Refresh] ✅ Loaded {initial_count} existing people")
        
        # Scan and update
        face_db = add_new_people(uploads_folder, face_app, face_db)
        
        # Save updated database
        save_face_db(face_db, face_db_path)
        final_count = len(face_db)
        
        print(f"[Refresh] ✅ Complete! {initial_count} → {final_count} people registered")
        return face_db
